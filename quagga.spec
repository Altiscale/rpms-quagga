...
BuildRequires:  kernel-headers >= 4.8
...
%prep
# the quagga zip pulled from github doesn't conform to any name
# the rpm build expects and shenanigans need to be perpetrated
# to put things in the right place
cd $RPM_BUILD_DIR/../
rm -rf $RPM_BUILD_DIR/
unzip  SOURCES/cmaster.zip
mv quagga-cmaster $RPM_BUILD_DIR
cd $RPM_BUILD_DIR
...
#./autogen.sh                                                                 
export CFLAGS="$RPM_OPT_FLAGS $CPPFLAGS -fno-strict-aliasing -fPIC"
...
if getent passwd %quagga_user >/dev/null 2>&1 ; then : ; else \
 ...
 || exit 1 ; fi
if /usr/bin/groups %quagga_user | /bin/grep -q %vty_group 2> /dev/null ; then : ; else \
 /usr/sbin/usermod -G %vty_group %quagga_user 2> /dev/null \
 || exit 1; fi
%endif
...
%endif
%attr(0755,root,root) /usr/share/cumulus/support/quagga
...
%changelog
* Fri Oct 20 2017 Ryan Tokarek - 1.0.0-cl3u10
- Updates source to Cumulus Network's github fork with bgp evpn excentions. 
